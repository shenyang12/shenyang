setwd("C:\\Users\\admin\\Desktop\\reAnalysisTP53\\11.pancancer SNV")
...
##输入文件：mc3.v0.2.8.PUBLIC.maf.gz，TCGA pancancer突变数据，下载自https://gdc.cancer.gov/about-data/publications/pancanatlas Mutations，下载地址http://api.gdc.cancer.gov/data/1c8cfe5f-e52d-41ba-94da-f15ea1337efc

#merged_sample_quality_annotations.tsv，下载自https://gdc.cancer.gov/about-data/publications/pancanatlas，下载地址http://api.gdc.cancer.gov/data/1a7d7be8-675d-4e60-a105-19d4121bdebf。跟FigureYa263panDiff、FigureYa265panCNV、FigureYa268panCNVexpr使用的是同一套数据，已经下载的小伙伴就不用重复下载了
...
panmaf1 <- panmaf[which(panmaf$Hugo_Symbol %in% genelist),]
panmaf1$Tumor_Sample_Barcode <- substr(panmaf1$Tumor_Sample_Barcode,1,15)
write.table(panmaf1, file = "maf of genes of interest.txt",sep = "\t",row.names = F,col.names = T,quote = F)

# 创建突变矩阵
panmaf1 <- read_tsv("maf of genes of interest.txt", comment = "#")
...
for (i in genelist) {
  message(i," starts...")
  submaf <- panmaf1[which(panmaf1$Hugo_Symbol == i),] # 取出当前基因的突变
