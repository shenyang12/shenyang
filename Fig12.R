...
setwd("C:\\Users\\Administrator\\Desktop\\GitHub\\Figs\\Fig12")
#引用包
library(limma)
rm(list=ls())
...
rt <- read.delim("./drugCor.txt",row.names = 1)

rt=na.omit(rt)#删除缺失值所在的行

drug=read.table("drug.txt",sep="\t",header=F)  
...
pdf("heatmap1.TP53.pdf",width =7,height =8)
