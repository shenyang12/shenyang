...
setwd("E:\\桌面临时文件\\TP53_Family _2\\15_P53_TIMER_heatmap/test/")        #设置工作目录
up <- read.table("TP53cor.resultpval.txt",sep = "\t",check.names = F,header = T,row.names=1)  #读取左上角的数据
dn <- read.table("TP63cor.resultcor.txt",sep = "\t",check.names = F,header = T,row.names=1)     #读取右下角数据
dn=t(dn)
up=t(up)
#设置颜色
colVector=c("#DDDDDD","#4169E1","#FFFFFF","#FF4500")
...
dev.off()
##颜色代码http://www.mamicode.com/info-detail-1448036.html
